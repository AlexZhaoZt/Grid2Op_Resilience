...
print('Pandapower backend version: ')
...
class ResilienceBackend(Backend):
    """
    INTERNAL
    ...
    .. code-block:: python
            ...
            import grid2op
            from grid2op.Backend import ResilienceBackend
    ...
    def __init__(self, detailed_infos_for_cascading_failures=False):
        ...
        # throws exception if more than self.contingency_security number of buses are isolated.
        # throws exception if there are more than self.max_islands number of islands.
        self.max_islands = 1000
    ...
    def load_grid(self, path=None, filename=None):
        ...
        #TODO: leave the space here to add multiple slack buses for island mode
        # self._grid.gen["slack"][2] = True
        ...
        try:
            with warnings.catch_warnings():
                ...
                if self._nb_bus_before is None:
                    ...
                    self._pf_init = "auto"
                # print(self._grid.gen["in_service"])
                ...
                if is_dc:
                    pp.rundceither(self._grid)
                    # pp.rundcopp(self._grid, check_connectivity=False)
                    self._nb_bus_before = None  # if dc i start normally next time i call an ac powerflow
                else:
                    # report = pp.diagnostic(self._grid)
                    # print(report)
                    pp.runac(self._grid, init=self._pf_init)
                    # pp.runpp(self._grid, check_connectivity=False, init=self._pf_init, numba=numba_)
                self.check_resilience_conditions()
                ...
                # if np.isnan(self.prod_v[:]).any() or np.isnan(self.prod_p[:]).any():
                ...
                self._topo_vect[:] = self._get_topo_vect()
                # print('converged:', self._grid.converged)
                # print('OPF_converged:', self._grid.OPF_converged)
                converged = self._grid.converged or self._grid.OPF_converged
    ...
    def get_num_islands(self):
        return self._grid.num_islands

    def get_num_isolated(self):
        return len(self._grid._isolated_buses) - self.get_nb_active_bus() * 1
